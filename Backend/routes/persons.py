...
def read_persons(skip: int = 0, limit: int = 50, db: Session = Depends(get_db)):
    db_persons = crud.persons.get_persons(db=db, skip=skip, limit=limit)
    return db_persons

@person.get("/person/{ID}", response_model=schemas.persons.Person, tags=["Personas"])
...
def update_person(ID: int, person: schemas.persons.PersonUpdate, db: Session = Depends(get_db)):
    ...
    if db_person is None:
        raise HTTPException(status_code=404, detail="Persona no existente, no se pudo actualizar")
    return db_person
...
def delete_person(ID: int, db: Session = Depends(get_db)):
    ...
    if db_person is None:
        raise HTTPException(status_code=404, detail="Persona no existente, no se pudo eliminar")
    return db_person
